...
const phaseDoc = `BE/docs/PHASES/PHASE${phaseNum}.md`
const logPath = 'BE/docs/compound-log.md'
...
2. BE/.claude/agents/${agentFile}.md 파일을 읽고, 그 안에 정의된 역할과 규칙을 그대로 따른다.
...
while (attempt < MAX_LOOPS) {
  ...
  testResult = await agent(
    withContext(
      'test',
      `${phaseDoc} 문서를 기준으로 방금 끝난 구현에 대한 체크리스트를 만들고, 테스트를 직접 실행해서 통과/실패를 구분해줘. 통과한 항목은 screenshots 폴더에 png 캡쳐를 남겨줘. verdict 필드에는 실패 항목이 하나라도 있으면 fail, 전부 통과면 pass를 넣고, report 필드에는 test.md에서 정의한 형식(통과/실패 목록) 그대로 담아줘.\n\n[coding 단계 결과]\n${codingResult}`
    ),
