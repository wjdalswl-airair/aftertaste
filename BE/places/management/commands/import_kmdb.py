...
from places.services import get_or_create_work_by_source, parse_yyyymmdd
...
class Command(BaseCommand):
    ...
    그대로 다 가져온다 (2026-09-01 결정). KMDB DOCID를 WorkSource에 기록해서, 다시 실행할 때
    제목 표기가 조금 달라도 같은 작품으로 이어진다. 이미 있는 작품은 KMDB 데이터로 덮어쓰지
    않는다 — 관리자가 고친 값을 지키기 위해서다 (docs/DETAIL_SPEC.md 6-1 #28).
    """
