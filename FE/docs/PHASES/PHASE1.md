...
**보류 → 제외 확정 (2026-08-29 보류, 2026-09-13 최종 제외 결정)**
- Apple 로그인: Firebase 콘솔에서 활성화하려면 Apple Developer Program 가입(연 $99)이 필요함을 확인. 실제로 버튼을 누르면 `auth/operation-not-allowed` 에러가 남 (계정 없음 → Firebase 콘솔에서 Apple 프로바이더 활성화 자체가 안 된 상태). 2026-08-29 당시엔 "나중에 계정이 생기면 활성화"로 보류했었는데, 2026-09-13 사용자가 **Apple 로그인은 아예 하지 않기로 최종 결정** — Google/Kakao 두 개만 지원한다(`src/lib/firebase.ts` 상단 주석에도 2026-09-04자로 이미 이 결정이 반영돼 있었음). Apple 버튼 코드는 애초에 커밋된 적이 없어서 별도로 지울 코드는 없음.
...
**남은 일**
- 없음 — Apple 로그인은 2026-09-13 최종적으로 지원하지 않기로 결정됨(위 참고)

## 완료 기준 체크리스트

- [x] Google 계정으로 로그인하면 서버에 회원이 생성되거나 조회된다 (2026-08-29 브라우저 확인)
- [x] N/A — Apple 로그인은 지원하지 않기로 최종 결정됨(2026-09-13). Google/Kakao 소셜 로그인만 지원.
