...
- 지금은 `src/hooks/useGeolocation.ts`가 브라우저 네이티브 권한 팝업만 띄운다.
- Figma 디자인 기준으로, 네이티브 팝업 요청 전에 "왜 위치 정보가 필요한지" 설명하는 앱 자체 모달을 추가한다. 모달에서 사용자가 동의하면 그때 `navigator.geolocation.getCurrentPosition`을 호출해 네이티브 팝업을 띄운다.
- 거부 시 재요청하지 않는 기존 규칙(PHASE2.md)은 유지한다.
...
- [ ] 카카오 버튼으로 실제 로그인이 되고, 애플 버튼은 화면에서 사라졌다
- [ ] 로그인 화면에 로고 이미지가 보인다 (더 이상 회색 placeholder 아님)
- [ ] 배너 데이터 연동 상태 확인 완료 (FE 코드 변경 없음 확인)
- [ ] 위치 정보 요청 전 커스텀 설명 모달이 뜨고, 동의해야 네이티브 권한 팝업이 뜬다
- [ ] 비로그인 상태에서 언어를 바꾸면 명소·작품 콘텐츠(설명 등)가 해당 언어로 보인다
- [ ] 리뷰에 신고 버튼이 있고, 신고가 정상 접수된다
- [ ] 관련 유닛 테스트(카카오 로그인 API 함수, 리뷰 신고 API 함수 성공/실패 처리) 통과 (Vitest)
