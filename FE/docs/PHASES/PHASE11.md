...
- ~~`pages/LoginPage.tsx`의 `Modal title="이용약관"` 안 placeholder를 실제 텍스트로 교체~~ — **당초 계획과 다르게 처리함(2026-09-13).** 그사이 마이페이지 "약관 모아보기"에서 이용약관·개인정보처리방침·위치기반서비스 이용약관 3종의 실제 페이지(`/terms/service`, `/terms/privacy`, `/terms/location`)가 먼저 만들어졌다. 로그인 화면에 모달을 별도로 다시 채우는 대신, 로그인 화면의 "이용약관"/"개인정보처리방침" 텍스트를 이 실제 페이지로 이동하는 `Link`로 바꿨다 — 모달 텍스트를 이중으로 관리할 필요가 없고, 개인정보처리방침도 (당초 범위 밖이었지만) 자연스럽게 같이 해결됨.
- 이 페이지들은 로그인 전에도 봐야 하므로(회원가입 전 약관 확인), `App.tsx`에서 `/terms`, `/terms/:slug` 라우트를 `RequireAuth` 밖으로 옮겼다 — 원래 마이페이지 진입 전제로 로그인 가드 안에 있었는데, 로그인 화면에서 링크를 걸면서 비로그인 접근이 필요해짐.
...
- [x] 로그인 화면에서 이용약관·개인정보처리방침을 누르면 실제 내용을 볼 수 있다 (모달 대신 `/terms/service`, `/terms/privacy` 실제 페이지로 이동, 2026-09-13)
- [x] 카카오 로그인이 실제로 완료된다 — BE가 code→access_token 교환을 구현해 더 이상 막혀있지 않음(2026-09-13 사용자 확인)
