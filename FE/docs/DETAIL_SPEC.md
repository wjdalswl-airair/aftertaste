...
- **검색은 촬영지(장소명) 기준만 지원한다(2026-09-13 사용자 결정).** 리뷰 응답에 이미 있는 `place_name`을 FE에서 대소문자 무시하고 부분일치로 걸러 보여준다 — BE 호출 없이 이미 불러온 리뷰만 대상이라, 아직 안 불러온 뒷 페이지의 리뷰는 "더보기"로 더 불러와야 검색 대상에 들어온다. 작품(work) 기준 검색은 리뷰 응답에 작품 정보 자체가 없어서(리뷰는 장소에만 연결) BE가 필드나 검색 파라미터를 추가해주기 전까진 지원하지 않는다.
...
- **작품 태그 입력 (2026-09-13 추가, UI만·서버 미연동)**: 사진 첨부 위에 작품 제목 입력창을 추가했다. **후보는 DB 전체 작품이 아니라 "이 명소에서 촬영한 작품"으로 제한한다** — `getPlaceDetail`이 이미 받아온 `place.works`(명소 상세 API의 등장 작품 목록)에서 입력한 글자를 포함하는 제목만 클라이언트에서 걸러 보여준다(2026-09-13 사용자 결정, 처음엔 통합검색 API로 DB 전체를 대상으로 했다가 이 명소 기준으로 좁혔다). 그래서 별도 API 호출·디바운스가 필요 없고, 통합검색의 검색 기록 로그 부작용도 없다. 후보를 고르면 `#작품명` 해시태그 칩으로 입력창 바로 아래에 쌓인다(칩은 X로 삭제 가능). **`ReviewInput`(`src/api/reviews.ts`)에 작품을 연결할 필드가 없다** — 리뷰는 BE에서 place에만 연결되고 work와는 연결 모델 자체가 없어서, 고른 작품은 화면에만 남고 "등록" 제출 시 서버로 전송되지 않는다. 실제로 리뷰-작품을 연결하려면 BE에 필드/API 추가가 필요하다(2026-09-13 확인, 검토 대기).
  - **할 일(BE 반영 후 진행, 2026-09-13 추가)**: 리뷰 피드(`ReviewFeedPage.tsx`) 검색창에 지금은 촬영지(장소명)만 매칭하는데, 작품 기준 검색도 추가하기로 했다. BE가 `ReviewSerializer`에 태그된 작품을 노출해줘야(위 이슈 3번) `ReviewItem`에 작품 정보가 실려서, `visibleReviews` 필터 조건에 작품 제목 매칭을 더할 수 있다. 그 전까진 응답에 매칭할 데이터 자체가 없어서 보류.
